...
static ACTIVE_SESSIONS: Lazy<Gauge> = Lazy::new(|| {
    register_gauge!("chat_active_sessions", "Número de sessões ativas").unwrap()
});
...
impl AuthManager {
    pub fn new(users_file_path: &str) -> Self {
        ...
        let db_path = std::env::var("DB_PATH")
            .unwrap_or_else(|_| users_file_path.replace(".json", ".db"));
        ...
        if std::env::var("SKIP_TOKEN_CLEANUP").is_err() && tokio::runtime::Handle::try_current().is_ok() {
            let sessions_clone = manager.sessions.clone();
            let ttl_secs: u64 = std::env::var("TOKEN_TTL_SECS").ok().and_then(|v| v.parse().ok()).unwrap_or(60 * 60 * 24);
            let cleanup_interval_secs: u64 = std::env::var("TOKEN_CLEANUP_INTERVAL_SECS").ok().and_then(|v| v.parse().ok()).unwrap_or(60);
            ...
            tokio::spawn(async move {
                loop {
                    ...
                    let to_remove: Vec<String> = sessions_clone
                        .iter()
                        .filter(|entry| {
                            now.signed_duration_since(entry.value().created_at).num_seconds() as u64 >= ttl.as_secs()
                        })
                ...
                match serde_json::from_str::<UsersFile>(&content) {
                    Ok(users_file) => {
                        ...
                        for record in users_file.users {
                            if let Ok(_) = self.db.insert_user(&record.username, &record.password_hash) {
                                migrated += 1;
                            }
                        }
                        info!("🔄 Migrados {} usuários de {} para SQLite", migrated, self.users_file_path);
    ...
    pub fn register(&self, username: &str, password: &str) -> Result<(), String> {
        ...
        let cost: u32 = std::env::var("BCRYPT_COST").ok().and_then(|v| v.parse().ok()).unwrap_or(12u32);
    ...
    pub fn login(&self, username: &str, password: &str) -> Result<String, String> {
        ...
        let rec = SessionRecord { username: username.to_string(), created_at: Utc::now() };
    ...
    pub fn validate_token(&self, token: &str) -> Option<String> {
        ...
        let username = if let Some(entry) = self.sessions.get(token) {
            ...
            if Utc::now().signed_duration_since(rec.created_at).num_seconds() as u64 >= ttl_secs {
                is_expired = true;
                None
            } else {
...
mod tests {
    ...
    fn test_register_and_login() {
        ...
        let token = auth.login("alice", "senha123").expect("Login deveria funcionar");
    ...
    async fn test_token_expiration_short_ttl() {
        ...
        assert!(auth.validate_token(&token).is_some(), "token deve ser válido imediatamente após login");
        ...
        assert!(result.is_none(), "O token deveria ter expirado, mas retornou: {:?}", result);
