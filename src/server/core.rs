...
impl ChatCore {
    ...
    pub fn handle_message(&self, msg: ChatMessage, addr: SocketAddr) {
        match msg {
            ChatMessage::Login { username, .. } => {
                warn!(
                    "Tentativa de login via handle_message para {}. Use register_user.",
                    username
                );
            }
                    ...
                    if let Some(username) = self.username_by_addr(&addr) {
                        if let Some(tx) = self.state.get_client_tx(&username) {
                            self.send_reliable(
                                ...
                                ChatMessage::Ack {
                                    kind: AckKind::Failed,
                                    info: "Token inválido ou expirado".to_string(),
                                    message_id: None,
                                },
                            );
                        }
                ...
                if let Some(username) = self.username_by_addr(&addr) {
                        ...
                        if let Some(tx) = self.state.get_client_tx(&username) {
                            self.send_reliable(
                                ...
                                ChatMessage::Ack {
                                    kind: AckKind::Failed,
                                    info: format!(
                                        "Limite de mensagens excedido ({} msg/s).",
                                        rate_limit_per_sec()
                                    ),
                                    message_id: None,
                                },
                    ...
                    if let Some(tx) = self.state.get_client_tx(&username) {
                        self.send_reliable(
                            ...
                            ChatMessage::Ack {
                                kind: AckKind::System,
                                info: format!("Entrou na sala {}", room),
                                message_id: None,
                            },
                        );
                    }
                ...
                if let Ok(true) = db.is_muted(&username) {
                    if let Some(tx) = self.state.get_client_tx(&username) {
                        self.send_reliable(
                            ...
                            ChatMessage::Ack {
                                kind: AckKind::Failed,
                                info: "🔇 Você está silenciado e não pode enviar mensagens"
                                    .to_string(),
                                message_id: None,
                            },
                        );
                ...
                if let Some(db) = &self.db {
                    ...
                    tokio::task::spawn_blocking(move || {
                        ...
                        match messages_result {
                            ...
                            Err(e) => {
                                ...
                                if let Some(tx) = state_clone.get_client_tx(&username_clone) {
                                    let err_msg = ChatMessage::Error(format!(
                    ...
                    if let Some(tx) = self.state.get_client_tx(&username) {
                        self.send_reliable(
                            tx,
                            ChatMessage::Error(
                                "Histórico não disponível (sem banco de dados)".to_string(),
                            ),
                        );
                    }
                }
            ...
            ChatAction::Logout { .. } => {
                ...
                if let Some(tx) = self.state.get_client_tx(&username) {
                    self.send_reliable(
                        ...
                        ChatMessage::Ack {
                            kind: AckKind::System,
                            info: "Logout realizado".to_string(),
                            message_id: None,
                        },
                    );
                }
                ...
                if let Some(db) = &self.db {
                    match db.is_admin(&username) {
                        Ok(true) => {
                            ...
                            // Log de moderação
                            let _ =
                                db.log_moderation("kick", &username, &target, Some(&reason), None);
                ...
                if let Some(db) = &self.db {
                    match db.is_admin(&username) {
                        Ok(true) => {
                            ...
                            // Banir no banco
                            let duration_opt = if duration_secs > 0 {
                                Some(duration_secs)
                            } else {
                                None
                            };
                ...
                if let Some(db) = &self.db {
                    match db.is_admin(&username) {
                        Ok(true) => {
                            ...
                            let details = format!("duração: {}s", duration_secs);
                            let _ =
                                db.log_moderation("mute", &username, &target, None, Some(&details));
                            ...
                            if let Some(tx) = self.state.get_client_tx(&target) {
                                self.send_reliable(
                                    ...
                                    ChatMessage::Ack {
                                        kind: AckKind::System,
                                        info: format!(
                                            "Você foi silenciado por {} segundos",
                                            duration_secs
                                        ),
                                        message_id: None,
                                    },
                        ...
                        Err(e) => {
                            ...
                            if let Some(tx) = self.state.get_client_tx(&username) {
                                self.send_reliable(
                                    tx,
                                    ChatMessage::Error(
                                        "Erro ao buscar lista de admins".to_string(),
                                    ),
                                );
                            }
                        }
                            ...
                            match db.get_moderation_logs(limit_val) {
                                ...
                                Err(e) => {
                                    ...
                                    if let Some(tx) = self.state.get_client_tx(&username) {
                                        self.send_reliable(
                                            tx,
                                            ChatMessage::Error("Erro ao buscar logs".to_string()),
                                        );
                                    }
                                }
                            }
                    ...
                    match db.search_messages(&query, limit_val, user_filter.as_deref()) {
                        ...
                        Err(e) => {
                            ...
                            if let Some(tx) = self.state.get_client_tx(&username) {
                                self.send_reliable(
                                    tx,
                                    ChatMessage::Error(format!("Erro na busca: {}", e)),
                                );
                            }
                        }
                    }
                    ...
                    match db.generate_download_token(&file_id, &to) {
                        ...
                        Err(e) => {
                            ...
                            if let Some(tx) = self.state.get_client_tx(&username) {
                                self.send_reliable(
                                    tx,
                                    ChatMessage::Error("Erro ao enviar arquivo".to_string()),
                                );
                            }
                        }
                    }
    ...
    pub fn register_user(&self, username: String, addr: SocketAddr, tx: Tx) {
        ...
        if let Err(err_msg) = ChatState::validate_username(&username) {
            ...
            self.send_reliable(
                tx.clone(),
                ChatMessage::Error(format!("Username inválido: {}", err_msg)),
            );
            return;
        }
        ...
        if self.state.is_username_taken(&username) {
            ...
            self.send_reliable(
                tx.clone(),
                ChatMessage::Error(format!("Username '{}' já está em uso", username)),
            );
            return;
        }
        ...
        self.broadcast(
            "Sistema".to_string(),
            format!("{} entrou na sala!", username),
            Utc::now(),
        );
    }
        ...
        match auth.login(&username, &password) {
            Ok(token) => {
                ...
                self.send_reliable(
                    ...
                    ChatMessage::Ack {
                        kind: AckKind::System,
                        info: "Login bem-sucedido".to_string(),
                        message_id: None,
                    },
                );
                        ...
                        match db_clone.get_pending_messages(&username_clone) {
                            Ok(pending) => {
                                if !pending.is_empty() {
                                    ...
                                    if let Some(user_tx) =
                                        state_clone.get_client_tx(&username_clone)
                                    {
                                    ...
                                    if !delivered_ids.is_empty() {
                                        if let Err(e) =
                                            db_clone.mark_messages_delivered(&delivered_ids)
                                        {
            ...
            Err(e) => {
                ...
                } else if e.to_lowercase().contains("password incorreto")
                    || e.to_lowercase().contains("senha incorreta")
                {
    ...
    pub fn handle_register(&self, username: String, password: String, _addr: SocketAddr, tx: Tx) {
        if self.auth.is_none() {
            ...
            self.send_reliable(
                ...
                ChatMessage::Ack {
                    kind: AckKind::Failed,
                    info: "Registro não suportado no servidor".to_string(),
                    message_id: None,
                },
            );
            return;
                ...
                match auth.login(&username, &password) {
                    Ok(token) => {
                        ...
                        self.send_reliable(
                            ...
                            ChatMessage::Ack {
                                kind: AckKind::System,
                                info: "Registro realizado e login efetuado".to_string(),
                                message_id: None,
                            },
                        );
                    }
                    Err(e) => {
                        ...
                        self.send_reliable(
                            ...
                            ChatMessage::Ack {
                                kind: AckKind::System,
                                info: "Registro realizado com sucesso".to_string(),
                                message_id: None,
                            },
                        );
                        self.send_reliable(
                            ...
                            ChatMessage::Ack {
                                kind: AckKind::Failed,
                                info: format!("Falha no login automático: {}", e),
                                message_id: None,
                            },
                        );
                    }
