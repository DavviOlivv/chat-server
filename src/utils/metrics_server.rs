...
use prometheus::{Encoder, TextEncoder, gather, register_counter, Counter};
...
async fn metrics_handler(req: Request<Body>) -> impl IntoResponse {
            ...
            Some(header_val) => {
                if let Ok(s) = header_val.to_str() {
                    ...
                    if s != expected_header {
                        ...
                        return (StatusCode::UNAUTHORIZED, [(CONTENT_TYPE, "text/plain; charset=utf-8")], "unauthorized".to_string());
                    }
                } else {
                    METRICS_UNAUTHORIZED_TOTAL.inc();
    ...
    if let Err(e) = encoder.encode(&metric_families, &mut buffer) {
        ...
        return (StatusCode::INTERNAL_SERVER_ERROR, [(CONTENT_TYPE, "text/plain; charset=utf-8")], body);
    }
    let body = String::from_utf8(buffer).unwrap_or_default();
    // TextEncoder format type is typically: "text/plain; version=0.0.4; charset=utf-8"
    (StatusCode::OK, [(CONTENT_TYPE, "text/plain; version=0.0.4; charset=utf-8")], body)
}

// Prometheus metric for unauthorized attempts to /metrics
static METRICS_UNAUTHORIZED_TOTAL: Lazy<Counter> = Lazy::new(|| {
    register_counter!("chat_metrics_unauthorized_total", "Total de tentativas não autorizadas ao endpoint /metrics").unwrap()
});

pub async fn run_metrics_server(bind: &str) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    let app = Router::new()
