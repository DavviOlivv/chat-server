...
use chat_serve::model::message::{ChatMessage, AckKind};
...
async fn connect_client(addr: &str) -> (BufReader<tokio::net::tcp::OwnedReadHalf>, tokio::net::tcp::OwnedWriteHalf) {
        ...
        match timeout(Duration::from_secs(2), reader.read_line(&mut line)).await {
            Ok(Ok(n)) if n > 0 => {
                if let Ok(msg) = serde_json::from_str::<ChatMessage>(&line) {
                    ...
                    match &msg {
                        ChatMessage::Ack { kind: AckKind::System, .. } => continue,
...
async fn test_login_and_broadcast() {
    ...
    // Login cliente 1
    send_msg(&mut writer1, &ChatMessage::Login { username: "alice".to_string(), password: "test".to_string() }).await;
    
    // Login cliente 2
    send_msg(&mut writer2, &ChatMessage::Login { username: "bob".to_string(), password: "test".to_string() }).await;
...
async fn test_dm_with_ack() {
    ...
    assert!(received_bob.is_ok() && received_bob.as_ref().unwrap().is_some(), "Bob não recebeu DM");
    
    match received_bob.unwrap().unwrap() {
        ChatMessage::Private { from, to, content, .. } => {
            assert_eq!(from, "alice");
            assert_eq!(to, "bob");
    ...
    assert!(ack.is_ok() && ack.as_ref().unwrap().is_some(), "Alice não recebeu ACK");
    
    match ack.unwrap().unwrap() {
        ChatMessage::Ack { kind, message_id: ack_id, .. } => {
...
async fn test_list_users() {
    ...
    send_msg(&mut writer1, &ChatMessage::ListRequest { from: "alice".to_string() }).await;
    ...
    match response.unwrap() {
        ChatMessage::ListResponse { users } => {
            assert!(users.len() >= 1, "Lista deveria ter pelo menos 1 usuário, tem {}", users.len());
            assert!(users.contains(&"alice".to_string()), "Alice não está na lista");
...
async fn test_room_join_and_text() {
    ...
    send_msg(&mut writer1, &ChatMessage::JoinRoom { 
        room: "#dev".to_string() 
    }).await;
    
    send_msg(&mut writer2, &ChatMessage::JoinRoom { 
        room: "#dev".to_string() 
    }).await;
    ...
    send_msg(&mut writer1, &ChatMessage::RoomText {
        from: "alice".to_string(),
        room: "#dev".to_string(),
...
async fn test_rate_limiting() {
    ...
    for i in 0..15 {
        send_msg(&mut writer1, &ChatMessage::Text {
            from: "alice".to_string(),
    ...
    assert!(found.is_ok() && found.unwrap(), "Rate limit não foi aplicado - não recebeu ACK Failed");
}
...
async fn test_message_deduplication() {
    ...
    assert!(first.is_ok() && first.as_ref().unwrap().is_some(), "Bob não recebeu primeira mensagem");
    ...
    assert!(second.is_err() || second.unwrap().is_none(), "Bob recebeu mensagem duplicada!");
}
...
async fn test_disconnect_cleanup() {
    ...
    send_msg(&mut writer1, &ChatMessage::Login { username: "alice".to_string(), password: "test".to_string() }).await;
    send_msg(&mut writer2, &ChatMessage::Login { username: "bob".to_string(), password: "test".to_string() }).await;
    send_msg(&mut writer3, &ChatMessage::Login { username: "charlie".to_string(), password: "test".to_string() }).await;
    ...
    for attempt in 0..5 {
        ...
        if let Ok(Some(ChatMessage::ListResponse { users })) = response {
            if !users.contains(&"bob".to_string()) {
                ...
                assert!(users.contains(&"alice".to_string()), "Alice não está na lista");
                assert!(users.contains(&"charlie".to_string()), "Charlie não está na lista");
                bob_gone = true;
                break;
            }
        }
    ...
    assert!(bob_gone, "Bob ainda está na lista após desconectar (após 5 tentativas)");
}
...
async fn test_dm_to_nonexistent_user() {
    ...
    match response.unwrap() {
        ChatMessage::Error(msg) => {
            assert!(msg.contains("ghost") || msg.contains("não encontrado") || msg.contains("not found"));
        }
