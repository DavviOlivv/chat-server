...
async fn test_offline_message_delivery() {
    ...
    send_msg(&mut writer_alice, &ChatMessage::Register {
    ...
    send_msg(&mut writer_bob_reg, &ChatMessage::Register {
    ...
    send_msg(&mut writer_bob, &ChatMessage::Login {
        username: "bob".to_string(),
        password: "pass456".to_string(),
    }).await;
    ...
    assert_eq!(received_messages.len(), 2, "Bob deveria receber 2 mensagens offline");
...
async fn test_offline_message_not_for_nonexistent_user() {
    ...
    send_msg(&mut writer, &ChatMessage::Register {
        username: "alice".to_string(),
        password: "pass123".to_string(),
    }).await;
...
async fn test_online_message_not_saved_as_offline() {
    ...
    let (mut reader_bob, mut writer_bob) = connect_client(&addr).await;
    send_msg(&mut writer_bob, &ChatMessage::Register {
        username: "bob".to_string(),
        password: "pass456".to_string(),
    }).await;
